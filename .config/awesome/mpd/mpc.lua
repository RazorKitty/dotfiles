...
function mpc:inc_volume(vol)
    self:send("status", function(success, status)
        if status.state ~= "stop" then
            self:send("setvol "..status.volume+vol)
        end     
    end)
end

function mpc:dec_volume(vol)
    self:send("status", function(success, status)
        if status.state ~= "stop" then
            self:send("setvol "..status.volume-vol)
        end
...
function mpc:set_random(state)
    if type(state) == 'number' and -1 < state and state < 2 then
        self:send('random '..state)        
    end
end

function mpc:set_single(state)
    if type(state) and -1 < state and state < 2 or state == 'oneshot' then
